...
// 🔥 Corregido: Importamos sendPasswordResetEmail
import {
...
import {
  ...
  AlertCircle,
  Stethoscope,
  Send, // 🔥 Añadimos el icono de enviar
} from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleRecuperarPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      setError(
        "Hubo un error. Verifica que el correo esté bien escrito y registrado.",
      );
    } finally {
      setCargando(false);
    }
  ...
  return (
    <div className="flex min-h-screen w-full font-sans bg-white">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, #162B54 0%, #2651A3 50%, #39ACB8 100%)",
        }}
      >
        ...
        <div className="z-10 max-w-md animate-in fade-in slide-in-from-left-4 duration-700 delay-150">
          ...
          <p className="text-blue-100 text-lg leading-relaxed font-light">
            Controla citas, pacientes, ingresos y a todo tu equipo desde una
            plataforma unificada, rápida y segura.
          </p>
        </div>
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-6 md:hidden">
              ...
              <span className="font-bold text-xl text-[#2651A3]">
                DentaSync
              </span>
            </div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
              {vistaRecuperar ? "Recuperar Acceso" : "Bienvenido de vuelta"}
            </h1>
          ...
          {mensajeExito && (
            <div className="flex items-start gap-3 mb-6 p-4 rounded-xl border bg-emerald-50 border-emerald-100 text-emerald-700 text-sm animate-in fade-in">
              <Mail className="w-5 h-5 shrink-0 mt-0.5" />
              <p className="font-medium leading-relaxed">{mensajeExito}</p>
            </div>
          )}

          {/* 🔥 LÓGICA CONDICIONAL: Mostrar Recuperar o Iniciar Sesión 🔥 */}
          {vistaRecuperar ? (
            ...
            >
              ...
              <button
                type="submit"
                disabled={cargando}
                className="w-full py-3.5 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-lg bg-[#39ACB8] hover:bg-[#2c8892] disabled:opacity-70"
              >
                {cargando ? (
                  "Enviando enlace..."
                ) : (
              ...
              <button
                ...
                className="w-full text-sm font-bold text-slate-500 hover:text-[#2651A3] transition-colors"
              >
